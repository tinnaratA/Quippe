define([
	...
	"dojo/_base/lang",
	"dojo/_base/array",
	"dojo/i18n!dojo/cldr/nls/islamic"
], function (IDate, regexp, string, i18n, date, lang, array) {
	var ilocale = lang.getObject("qx.date.islamic.locale", true);

	// Format a pattern without literals
	var formatPattern = function(dateObject, bundle, locale, fullYear,  pattern) {
		...
		return pattern.replace(/([a-z])\1*/ig, function(match){
			...
			switch(c){
				...
				case 'z':
					...
					s = date.getTimezoneName(dateObject.toGregorian());
					if(s){ break; }
					l = 4;
				...
				case 'Z':
					...
					break;
				default:
					throw new Error("qx.date.islamic.locale.formatPattern: invalid pattern char: "+pattern);
			}
	...
	ilocale.format = function(/*islamic.Date*/dateObject, /*Object?*/options){
		...
		var bundle = this._getIslamicBundle(locale);
		var str = [];
	...
	ilocale.parse = function(/*String*/value, /*Object?*/options){
		...
		var regex = info.regexp.replace(/[\u200E\u200F\u202A\u202E]/g, ""); //remove bidi non-printing chars from the pattern
		...
		if(!match){
			console.debug("qx.date.islamic.locale.parse: value  "+value+" doesn't match pattern   " + re);
			return null;
		} // null
		...
		var mLength = 0;
		var widthList = ["abbr", "wide", "narrow"];
		var valid = array.every(match, function(v, i){
			if(!i){return true;}
				...
				case 'M':
					if(l>2){
						...
						if(!options.strict){
							...
							months = array.map(months, function(s){ return s ? s.replace(".","").toLowerCase() : s; } );
						}
	...
	var _processPattern = function(pattern, applyPattern, applyLiteral, applyAll) {
	...
	var _buildDateTimeRE = function(tokens, bundle, options, pattern) {
	...
	ilocale.getNames = function(/*String*/item, /*String*/type, /*String?*/context, /*String?*/locale, /*islamic Date Object?*/date){
		...
		var label;
		var lookup = this._getIslamicBundle(locale);
