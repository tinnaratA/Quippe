define([
	...
	return declare("qx.date.islamic.Date", null, {
		...
		//
		// example:
		// |	dojo.require("qx.date.islamic.Date");
		// |
		// |	var date = new qx.date.islamic.Date();
		...
		constructor: function() {
			...
			//
			// example:
			// |		var date1 = new qx.date.islamic.Date();
			// |
			// |		var date2 = new qx.date.islamic.Date("12\2\1429");
			// |
			// |		var date3 = new qx.date.islamic.Date(date2);
			// |
			// |		var date4 = new qx.date.islamic.Date(1429,2,12);
		...
		toGregorian: function() {
			...
			// example:
			// |		var dateIslamic = new qx.date.islamic.Date(1429,11,20);
			...
			var tjd = (this._GREGORIAN_EPOCH - 1) + (365 * (year - 1)) + Math.floor((year - 1) / 4)
				- (Math.floor((year - 1) / 100)) + Math.floor((year - 1) / 400) + Math.floor((739 / 12)
					+ ((date.isLeapYear(new Date(year, 3, 1)) ? -1 : -2)) + 1);

			var leapadj = ((wjd < tjd) ? 0 : (date.isLeapYear(new Date(year, 3, 1)) ? 1 : 2));
			...
			var tjd2 = (this._GREGORIAN_EPOCH - 1) + (365 * (year - 1))
				...
				+ Math.floor((year - 1) / 400) + Math.floor((((367 * month) - 362) / 12)
					+ ((month <= 2) ? 0 : (date.isLeapYear(new Date(year, month, 1)) ? -1 : -2)) + 1);
		...
		fromGregorian: function( /*Date*/gdate) {
			...
			// example:
			// |		var dateIslamic = new qx.date.islamic.Date();
			...
			var julianDay = (this._GREGORIAN_EPOCH - 1) + (365 * (gYear - 1)) + Math.floor((gYear - 1) / 4)
				...
				+ Math.floor((((367 * (gMonth + 1)) - 362) / 12)
					+ (((gMonth + 1) <= 2) ? 0 : (date.isLeapYear(date) ? -1 : -2)) + gDay);
