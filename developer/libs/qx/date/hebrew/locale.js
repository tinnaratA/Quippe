define([
	...
	"./Date",
	"./numerals",
	"dojo/i18n!dojo/cldr/nls/hebrew"
], function(lang, arr, dd, i18n, regexp, string, HDate, numerals) {
	var hlocale = lang.getObject("qx.date.hebrew.locale", true);
			...
			switch (c) {
				...
				case 'M':
					...
					if (l < 3) {
						...
						var monthNames = this.getNames('months', widthList[l - 3], 'format', locale, dateObject);
						s = monthNames[m];
					}
					break;
				case 'd':
				...
				case 'z':
					...
					break;
				default:
					throw new Error("qx.date.hebrew.locale.formatPattern: invalid pattern char: " + pattern);
			}
			if (pad) {
				s = string.pad(s, l);
			}
	...
	hlocale.format = function( /*hebrew.Date*/dateObject, /*object?*/options) {
		...
		var bundle = this._getHebrewBundle(locale);
		var str = [];
	...
	hlocale.parse = function( /*String*/value, /*object?*/options) {
		...
		if (!match) {
			console.debug("qx.date.hebrew.locale.parse: value  " + value + " doesn't match pattern   " + re);
			return null;
		} // null
		...
		var mLength = 0;
		var widthList = ["abbr", "wide", "narrow"];
		var valid = arr.every(match, lang.hitch(this, function(v, i) {
			if (!i) {
				return true;
			}
			...
			switch (token.charAt(0)) {
				...
				case 'M':
					...
					if (l > 2) {
						...
						var months = this.getNames('months', widthList[l - 3], 'format', locale, new HDate(5769, 1, 1)),
						    leapmonths = this.getNames('months', widthList[l - 3], 'format', locale, new HDate(5768, 1, 1));
						if (!options.strict) {
	...
	hlocale.getNames = function( /*String*/item, /*String*/type, /*String?*/context, /*String?*/locale, /*dojox.date.hebrew.Date?*/date) {
		...
		var label,
		    lookup = this._getHebrewBundle(locale),
