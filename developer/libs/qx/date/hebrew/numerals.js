define([
	...
	var hnumerals = lang.getObject("qx.date.hebrew.numerals", true);

	hnumerals.getYearHebrewLetters = function(/*Number */ year){
		// summary: converts the year from an integer to Hebrew numerals.
		//
		// example:
		// |		var date1 = new qx.date.hebrew.Date();
		// |
		// |		document.writeln(qx.date.hebrew.numerals.getYearHebrewLetters(date1.getFullYear());
	...
	hnumerals.parseYearHebrewLetters  = function(/*String hebrew year*/ year){
		...
		//
		// example:
		// |		var date = new qx.date.hebrew.Date();
		// |        	date.setFullYear(qx.date.hebrew.numerals.parseYearHebrewLetters('\u05ea\u05e9\u05e1\u05f4\u05d7'));
	...
	hnumerals.getDayHebrewLetters =  function(day, /*boolean?*/ nogrsh){
		...
		// |		var date1 = new qx.date.hebrew.Date();
		// |
		// |		document.writeln(qx.date.hebrew.numerals.getDayHebrewLetters(date1.getDay());

		return transformChars(convertNumberToStr(day), nogrsh); // String
	};
	
	qx.date.hebrew.numerals.parseDayHebrewLetters =  function(/*String hebrew*/ day){
		// summary: converts the string containing a Hebrew numeral to an integer
		//
		// example:
		// |		var date1 = new qx.date.hebrew.Date();
		// |
		// |		date1.setDate(qx.date.hebrew.numerals.parseDayHebrewLetters('\u05d0')); // ALEPH
	...
	hnumerals.getMonthHebrewLetters =  function(/*int*/month){
		...
		// |		var date1 = new qx.date.hebrew.Date();
		// |
		// |		document.writeln(qx.date.hebrew.numerals.getMonthHebrewLetters(date1.getMonth());
	...
	hnumerals.parseMonthHebrewLetters = function(/*String*/monthStr){
		...
		// example:
		// |		var date = new qx.date.hebrew.Date();
		// |            var number = qx.date.hebrew.numerals.parseMonthHebrewLetters("\u05ea\u05de\u05d5\u05d6"); // Tammuz
		// |		date.setMonth(number);
			
		//month number from 0 to 12
		var monnum = this.parseDayHebrewLetters(monthStr) - 1;
