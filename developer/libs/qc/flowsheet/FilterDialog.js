define([
	...
	"dojo/on",
    "dojo/query",
    "dojo/text!qc/flowsheet/templates/FilterDialog.htm",
    "dojo/topic",
    "qc/_core",
    "qc/ListView"
], function (_TemplatedMixin, _WidgetBase, _WidgetsInTemplateMixin, Button, CheckBox, RadioButton, popup, registry, TooltipDialog, declare, lang, domGeometry, on, query, FilterDialogTemplate, topic, core, ListView) {
    return declare("qc.flowsheet.FilterDialog", [TooltipDialog, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
        ...
        loadFilter: function (noteEditor, filter) {
            ...
            query('input', this.domNode).forEach(function (node) {
                if (node.type == 'checkbox') {
                    ...
                    registry.getEnclosingWidget(node).set('checked', (filter[node.name] || 'false').toString() == node.value.toString());
                };
            });
        ...
        getSettingName: function (propertyName) {
            return 'fsFilter_' + propertyName;
        },
    
    
        isDefaultFilter: function (filter) {
            for (var p in filter) {
                if ((core.settings[this.getSettingName(p)] || false) !== (filter[p] || false)) {
                    return false;
                };
            };
            return true;
        ...
        onDataChanged: function () {
            ...
            query('input', this.currentOptionsTable).map(registry.getEnclosingWidget).forEach(function (x) { x.set('disabled', filter.currentLevel == 'all') });
        },
        ...
        onSaveDefault: function () {
            ...
            var settings = {};
            for (var p in filter) {
                settings[this.getSettingName(p)] = filter[p];
                core.settings[this.getSettingName(p)] = filter[p];
            };
        ...
        onApply: function () {
            var filter = this.getFilter();
            topic.publish('/flowsheet/SetFilter', filter);
            this.onCancel();
        },
